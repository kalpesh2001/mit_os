...
usertrap(void)
{
  ...
  if(which_dev == 2) {
      p->counter++;
      if (p->timer > 0 && (p->counter == p->timer)){
       //printf("Timer intrupt called\n");
       //Save the state
        savestate();
       p->saveframe->epc = p->trapframe->epc ;//Restore to same instruction. 
       p->trapframe->epc = (uint64)p->handler;
     }
      else if (p->timer > 0 && p->counter < p->timer);
    yield();
  }
  usertrapret();
}
